...
//! resumable / `markq models pull` UX lands in Phase 7.
...
/// Curated set of GGUFs markq knows how to fetch. Each phase that needs a new
/// model adds an entry. v1 only ships the embedder; the reranker (Phase 6)
/// and HyDE generator (Phase 13) will join this list.
...
impl KnownModel {
    ...
    /// download. Phase 7's `markq doctor` will surface unpinned models.
    pub fn sha256(self) -> Option<&'static str> {
        match self {
            // TODO(phase 7): record the digest of the Qwen3-Embedding GGUF
