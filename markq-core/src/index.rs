//! The `Index` trait. Backend-agnostic by design — Phase 1 ships the LanceDB
...
/// collection filtering pushes down through the same call shape — Phase 10
...
pub trait Index: Send + Sync {
    ...
    /// Phase 1 returns Ok with a no-op when the row count is zero so the
    ...
    /// pre-fusion ranked lists. RRF (Phase 5) fuses these in markq-core.
    async fn hybrid(
        &self,
        query: &str,
        embedding: &[f32],
    ...
    /// Phase 8 wires the threshold logic.
