...
//! into this in Phase 1; the suite is structured so a second backend (if
...
/// Run the full Phase 1 contract suite against a freshly-opened backend.
/// Caller is responsible for tearing down the underlying storage.
pub async fn run_contract<I: Index>(idx: &I) {
    // Schema is the canonical Phase 1 shape.
    ...
    // Phase 1: query methods are wired but return empty. The contract is
    // "no panic, returns an empty Vec" so Phase 3+ can light them up
    ...
    // once Phase 4 wires the real KNN path, a mis-sized vector would start
